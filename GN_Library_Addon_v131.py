...
bl_info = {
    ...
    "wiki_url": "https://github.com/KoreTeknology/Blender-GN-Library-Addon",
    "warning": "",
    "category": "Node",
}
...
class NodeLibraryPrefs(AddonPreferences):
    bl_idname = __name__
    ...
    def draw(self, context):
        ...
        if self.show_menu_list_projectpath:
            box.prop(self, "userlib_pro1_path")
...
class NODE_MT_ButtonsSub(bpy.types.Menu):
    bl_label = 'Generators'
...
class NODE_MT_library_add(Menu):
    ...
    def draw(self, context):
        ...
        dirpath = gn_userlib_category1_path(context)
        if dirpath != "":
            layout.menu(NODE_MT_ButtonsSub.bl_idname, icon="NODETREE")
        else:
